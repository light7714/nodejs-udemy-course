...
exports.postReset = (req, res, next) => {
	...
	crypto.randomBytes(32, (err, buffer) => {
		...
		User.findOne({ email: req.body.email })
			...
			.then((result) => {
				//*if block because when user doesnt exist above, then after redirecting we dont wanna do anything else and we will get result as null
				if (result) {
					res.redirect('/');
					return transporter.sendMail({
						to: req.body.email,
...
exports.getNewPassword = (req, res, next) => {
	...
	User.findOne({
		resetToken: token,
		//*resetTokenExpiration in db was stored as 1 hour above the time when it was stored, so if now time is more than 1 hour of resetTokenExpiration time, then condition wont match ($gt-> greater than)
